...
def create_order(positions):
    ...
    requests.post("https://api.aqsi.ru/pub/v2/Orders/simple", headers=headers, json=params)
...
def get_all_orders():
    ...
    requests.get("https://api.aqsi.ru/pub/v2/Orders", headers=headers, params=params)
...
def del_good(del_id):
    requests.delete(f"https://api.aqsi.ru/pub/v2/Goods/{del_id}", headers=headers)
...
def add_good(new_id, name, original_price, price, type, current_folder_id):
    ...
    requests.post("https://api.aqsi.ru/pub/v2/Goods", headers=headers, json=params)


def update_dir(folder):
    requests.put("https://api.aqsi.ru/pub/v2/GoodsCategory", headers=headers, json=folder)


def update_good(item):
    requests.put("https://api.aqsi.ru/pub/v2/Goods", headers=headers, json=item)
...
def add_folder(new_id, new_dir_name, parent_id):
    ...
    requests.post("https://api.aqsi.ru/pub/v2/GoodsCategory", headers=headers, json=params)


def del_folder(delete_dir):
    requests.delete(f"https://api.aqsi.ru/pub/v2/GoodsCategory/{delete_dir['id']}", headers=headers)
